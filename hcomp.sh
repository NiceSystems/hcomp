...
then
	HADOOP_TREE=~/.hadoop_tree_structure.bck
		...
		# check that the cache exists , is not still updated (not changed in last 2 seconds) and not too out of date (not more then 30 minutes old)
	if [ -f ${HADOOP_TREE} ] &&  \
	(( $((`stat -c %Y ${HADOOP_TREE}` + 2)) < $((`date +%s` )) )) && \
	(( $((`stat -c %Y ${HADOOP_TREE}` + 1800)) > $((`date +%s` )) ))
...
then
	...
	# recompute the cache ( in bg)only if 600 secs passed
	if	[ -f ~/.hadoop_tree_structure ] &&  \
	(( $((`stat -c %Y ~/.hadoop_tree_structure` + 600)) < $((`date +%s` )) ))
	then
		(hadoop fs -lsr / | awk '{print $8"completetmpahretzzkk"$1}' | sed 's/completetmpahretzzkkd.*$/\//' | sed 's/completetmpahretzzkk-.*$//' | sed 's/completetmpahretzzkkFound//' > ~/.hadoop_tree_structure &)
	fi
	...
	# create a backup if 2 seconds pass
	if	[ -f ~/.hadoop_tree_structure.bck ] &&  \
	(( $((`stat -c %Y ~/.hadoop_tree_structure.bck` + 2)) < $((`date +%s` )) ))
	then
		(cp -f ~/.hadoop_tree_structure ~/.hadoop_tree_structure.bck &)
	fi
else
